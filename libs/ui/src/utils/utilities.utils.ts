...
import { IChainData } from '../interfaces/app-config.inrerface';
...
export function getChainData(chainId: number): IChainData {
  ...
  // const API_KEY = getEnv('REACT_APP_INFURA_PROJECT_ID');
...
export function verifyFields(params: any[], keys: any[]) {
  ...
  if (naStr.length !== 0) {
    throw new Error(
      `[verifyFields] Params missing needed keys. Params: ${prettyPrint(params)}, keys: ${prettyPrint(keys)}`
    );
  }
