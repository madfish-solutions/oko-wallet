...
const genericErc20Abi = require('../constants/erc20Abi.json');
...
export const getGasTokenBalance$ = (network: NetworkInterface, pkh: string): Observable<string> => {
  const { rpcUrl, gasTokenMetadata: { decimals } } = network;
...
export const getTokenBalance$ = (network: NetworkInterface, pkh: string, token: TokenMetadata): Observable<string> => {
  ...
  switch (specificNetworkId) {
    case 'tezos':
      ...
      return from(tezosToolkit.contract.at(tokenAddress)).pipe(
          switchMap((contract) => {
              return from(contract.views.balance_of([{ owner: tzAddress, token_id: '0' }]).read()).pipe(
                  map((response) => {
                    return formatUnits(response[0].balance.toFixed(), decimals)
                  })
              );
          }),
      );
  ...
  return (from(contract.balanceOf(pkh)) as Observable<string>)
      .pipe(map(balance => formatUnits(balance, decimals)));
};
