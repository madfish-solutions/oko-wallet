...
import { DEFAULT_ACTIVE_INDEX, DEFAULT_CHAIN_ID, ENTROPY_KEY, MNEMONIC_KEY } from '../constants/defaults';
import { getAppConfig } from '../constants/get-app-config';
import { getLocal, setLocal } from '../utils/local.utils';
import { getChainData } from '../utils/utilities.utils';
...
export class WalletController {
  ...
  public getAccounts(count = getAppConfig().numberOfAccounts) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      console.log('getAccounts:', wallet, i);
  ...
  public generateWallet(index: number) {
    this.wallet = ethers.Wallet.fromMnemonic(MNEMONIC_KEY, this.getPath(index));
    console.log('generateWallet', this.getPath(index), this.wallet);
  ...
  public update(index: number, chainId: number): ethers.Wallet {
    ...
    const wallet = this.generateWallet(0);
