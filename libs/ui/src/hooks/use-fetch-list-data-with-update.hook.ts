...
const lastUpdateDataTimeCache: Record<string, Record<string, number>> = {};

export const useFetchListDataWithUpdate = (
  ...
  const fetchDataAndSetLastUpdateTime = (timestamp: number, cache?: 'only-cache') => {
    lastUpdateDataTimeCache[pkh][currentTokenAddress] = isDefined(cache) ? timestamp : new Date().getTime();
    callback(timestamp);
  };
  ...
  useEffect(() => {
    ...
    if (!lastUpdateDataTimeCache.hasOwnProperty(pkh)) {
      lastUpdateDataTimeCache[pkh] = {};
    }
    ...
    if ((lastUpdateDataTimeCache[pkh][currentTokenAddress] ?? 0) === 0) {
      fetchDataAndSetLastUpdateTime(new Date().getTime());
    ...
    const gapTime = currentTime - (lastUpdateDataTimeCache[pkh][currentTokenAddress] ?? 0);
    ...
    if (gapTime > slippageTime) {
      ...
      fetchDataAndSetLastUpdateTime(lastUpdateDataTimeCache[pkh][currentTokenAddress] ?? 0, 'only-cache');
